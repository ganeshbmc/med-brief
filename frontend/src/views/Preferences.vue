      ...
      <div class="col-md-10 col-lg-8">
        <div class="section-card p-4 p-md-5">
          <div class="d-flex align-items-center gap-3 mb-4">
            ...
            <div>
              <h2 class="fw-bold text-warm-dark mb-1">Preferences</h2>
              <p class="text-muted mb-0">Customize your reading experience.</p>
            </div>
          </div>
          ...
          <form v-else @submit.prevent="handleSave">
            <div class="mb-4">
              ...
              <div class="btn-group w-100" role="group" aria-label="Font size">
                <input type="radio" class="btn-check" id="fs-small" value="small" v-model="prefs.fontSize" :disabled="saved">
                <label class="btn btn-outline-secondary" for="fs-small">Small</label>
                
                <input type="radio" class="btn-check" id="fs-medium" value="medium" v-model="prefs.fontSize" :disabled="saved">
                <label class="btn btn-outline-secondary" for="fs-medium">Medium</label>
                
                <input type="radio" class="btn-check" id="fs-large" value="large" v-model="prefs.fontSize" :disabled="saved">
                <label class="btn btn-outline-secondary" for="fs-large">Large</label>
              </div>
              <div class="form-text text-muted mt-1">
                Choose your preferred reading font size · Current: <strong>{{ prefs.fontSize }}</strong>
              </div>
            </div>

            <div class="mb-4">
              <label class="form-label fw-semibold">Line Spacing</label>
              <div class="btn-group w-100" role="group" aria-label="Line spacing">
                <input type="radio" class="btn-check" id="ls-normal" value="normal" v-model="prefs.lineSpacing" :disabled="saved">
                <label class="btn btn-outline-secondary" for="ls-normal">Normal</label>
                
                <input type="radio" class="btn-check" id="ls-relaxed" value="relaxed" v-model="prefs.lineSpacing" :disabled="saved">
                <label class="btn btn-outline-secondary" for="ls-relaxed">Relaxed</label>
              </div>
              <div class="form-text text-muted mt-1">
                Normal (1.5) or Relaxed (1.8) line height · Current: <strong>{{ prefs.lineSpacing }}</strong>
              </div>
            </div>

            <div class="mb-4">
              <label class="form-label fw-semibold">Default Date Range</label>
              <select class="form-select" v-model="prefs.defaultDays" :disabled="saved">
                <option :value="1">Last 24 hours (Daily)</option>
                <option :value="3">Last 3 days (Midweek)</option>
              ...
              <div class="form-text text-muted mt-1">
                Default time period when generating new briefs · Current: <strong>{{ prefs.defaultDays }} days</strong>
              </div>
            </div>
